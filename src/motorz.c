...
int main(int argc, char **argv){
    ...
    while(1){
        ...
        if (fd_ins < 0) perror("Error opening ins-mz fifo");
        ...
        if (retval < 0) perror("Error in select");
            ...
            if (FD_ISSET(fd_cmd, &rfds)){
                ...
                if (read(fd_ins, buf, SIZE_MSG) < 0) perror("Error reading from ins-mz fifo");
            ...
            if (write(fd_ins, buf, 4*sizeof(char) + 1) < 0) perror("Error writing to ins-mz fifo");
