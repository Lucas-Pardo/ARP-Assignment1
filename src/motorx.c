...
int main(int argc, char **argv){
    ...
    if (fd_cmd < 0 && errno != EINTR) perror("Error opening cmd-mx fifo");
    int fd_world = open(world_fifo, O_WRONLY);
    if (fd_world < 0 && errno != EINTR) perror("Error opening world-mx fifo");
    int fd_watch = open(watch_fifo, O_WRONLY);
    if (fd_watch < 0 && errno != EINTR) perror("Error opening watch-mx fifo");
